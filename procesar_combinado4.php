...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_FILES['archivoActa']) && $_FILES['archivoActa']['error'] == 0) {
        $directorioDestino = 'img/mantenciones_fisicas/';
    ...
    if ($fecha && $observacion && $archivoActa) {
        ...
        $sql = "INSERT INTO mantencion_fisica (fecha, observacion, archivo) VALUES (?, ?, ?)";
        $stmt = $conn->prepare($sql);
        if (!$stmt) {
            echo "<script>alert('Error al preparar la consulta'); window.location.href = 'mantenciones2.php';</script>";
            $conn->close();
            exit();
        }
        ...
        if (!$stmt->execute()) {
            echo "<script>alert('Error al insertar el registro'); window.location.href = 'mantenciones2.php';</script>";
            $stmt->close();
            $conn->rollback();
            $conn->close();
            exit();
        ...
        $stmt->close();
        $conn->commit();
        echo "<script>alert('Registro insertado y documento subido correctamente.'); window.location.href = 'mantenciones2.php';</script>";
    } else {
        echo "<script>alert('Faltan datos requeridos.'); window.location.href = 'mantenciones2.php';</script>";
        $conn->rollback();
    }
    $conn->close();
} else {
    header("Location: mantenciones2.php");
    exit();
}
?>

            ...
            <div id="content" >
                    ...
                    <div class="row">
                        <div class="col-lg-12">
                            <div class="card shadow mb-4">
                                ...
                                $id_usuario_autenticado = $_SESSION['id']; // Asegúrate de usar el nombre correcto de la variable de sesión
                                ...
                                // Consulta para obtener las solicitudes relacionadas con el usuario autenticado como jefe
                                ...
                                // Consulta SQL con OFFSET y LIMIT
                                $query_solicitudes = "SELECT s.id_solicitud, s.fecha_solicitud, s.justificacion, s.estado, u.first_name
                                ...
                                $sql_total_registros = "SELECT COUNT(*) AS total FROM solicitudes";
                                ...
                                $stmt_solicitudes->bind_param("iii", $id_usuario_autenticado, $offset, $registros_por_pagina);
                                $stmt_solicitudes->execute();
                                $result_solicitudes = $stmt_solicitudes->get_result();

                                // Cerrar la declaración preparada de solicitudes
                                $stmt_solicitudes->close();
                                ?>
                                <div class="card-body">
                                    <div class="container mt-5">
                                        <h1>Solicitudes para Jefe</h1>
                                        <table id="example" class="table table-striped">
                                            ...
                                            <tbody>
                                                ...
                                                while ($row = $result_solicitudes->fetch_assoc()) {
                                                    echo "<tr data-id='" . $row['id_solicitud'] . "'>";
                                                        ...
                                                        echo '<td>';
                                                        echo '<button class="btn btn-success btn-aprobar" data-id="' . $row['id_solicitud'] . '" data-action="aprobar">Aprobar</button>';
                                                        echo '<button class="btn btn-danger btn-rechazar" data-id="' . $row['id_solicitud'] . '" data-action="rechazar">Rechazar</button>';
                ...
                <script>
                    ...
                    $(document).ready(function() {
                        // Manejar clics en los botones de "Aprobar" y "Rechazar"
                        $('.btn-aprobar, .btn-rechazar').click(function() {
                            ...
                            $('#confirmationModal .modal-body').html('¿Estás seguro de ' + action + ' esta solicitud?');

                            // Obtener el nombre del usuario que realizó la acción
                            var userName = 'Usuario no identificado';
                            if (action === 'aprobar') {
                                userName = $('.btn-aprobar[data-id="' + requestId + '"]').data('user-name');
                            } else if (action === 'rechazar') {
                                userName = $('.btn-rechazar[data-id="' + requestId + '"]').data('user-name');
                            }

                            // Actualizar el título del modal
                            $('#confirmationModal .modal-title').html('Confirmación de ' + userName);

                            $('#confirmationModal').modal('show');

                            // Actualizar el estado de la solicitud
                            actualizarEstadoSolicitud(requestId, action);

                            // Manejar clic en el botón de aceptar del modal
                            $('#confirmActionBtn').click(function() {
                                // Cerrar el modal
                                $('#confirmationModal').modal('hide');
                            });

                            // Manejar clic en el botón de cancelar del modal
                    ...
                    function actualizarEstadoSolicitud(idSolicitud, accion) {
                        // Enviar una solicitud AJAX para actualizar el estado de la solicitud
                        $.ajax({
                            type: 'POST',
                            url: 'aprobar_rechazar.php',
                            data: { id_solicitud: idSolicitud, accion: accion },
                            success: function(response) {
                                
                                //location.reload();
                            },
                            error: function(xhr, status, error) {
                                // Manejar errores que ocurran durante la solicitud AJAX
                                console.error(error);
